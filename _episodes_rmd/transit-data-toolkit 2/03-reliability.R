...
rawdata <- read.csv(file="./transit-data-toolkit 2/TDashboardData_reliability_20160301-20160331.csv", head=TRUE,sep=",")
#View(rawdata)
...
#Remove columns of data we don't need -> NOTE: do need STOP
...
stations <- read.csv("./transit-data-toolkit 2/mbta_stations.csv")
#View(stations)
...
mbta_subway <- ggmap(mbta_map) + 
  geom_point(data=joindata, aes(x=LONGITUDE, y=LATITUDE, color=rely), size=3, shape=15) + 
